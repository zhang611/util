...
class camera {
   public:
    ...
    double defocus_angle = 0;  // Variation angle of rays through each pixel // 相机镜头模糊程度（景深）
    double focus_dist =
    ...
    void render(const hittable& world) {
        ...
        if (!file) {
            std::cerr << "Error: Could not open output.ppm" << std::endl;
            return;
        }
   ...
   private:
    ...
    void initialize() {
        ...
        auto viewport_height =
            2 * h * focus_dist;  // 视口高度 = 2 * h(是tan(fov/2)的值) *
                                 // focus_dist(焦距)
        auto viewport_width =
        ...
        // frame.
        w = unit_vector(lookfrom - lookat);  // 0,0,0看向0,0,-1,z轴
        u = unit_vector(cross(vup, w));      // 0,1,0和z轴叉乘,得到x轴
        v = cross(w, u);
        ...
        // pixel.
        pixel_delta_u = viewport_u / image_width;  // 两个像素之间的水平距离
