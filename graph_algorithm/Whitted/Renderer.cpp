...
Vector3f refract(const Vector3f &I, const Vector3f &N, const float &ior) {
    float cosi = clamp(-1, 1, dotProduct(I, N));  // 入射角和法线夹角的余弦值
    float etai = 1, etat = ior;  // 折射率
    Vector3f n = N;
    if (cosi < 0) {        // 光从外往里射
        cosi = -cosi;
    } else {
    ...
    return k < 0 ? 0 : eta * I + (eta * cosi - sqrtf(k)) * n;  // 小于0时全反射
}
...
// 使用 std::optional<hit_payload>可选值容器 处理 可能无交点的情况。
std::optional<hit_payload> trace(
    const Vector3f &orig, const Vector3f &dir,
    const std::vector<std::unique_ptr<Object> > &objects) {
    float tNear = kInfinity;
    std::optional<hit_payload> payload;
    // 遍历场景中的所有对象，检查光线是否与它们相交
    // 找到最近的相交点，并返回碰撞信息 (hit_payload)。
    for (const auto &object : objects) {
        float tNearK = kInfinity;
        uint32_t indexK;
        Vector2f uvK;
        if (object->intersect(orig, dir, tNearK, indexK, uvK) &&
            tNearK < tNear) {
            payload.emplace();  // 原地构造
            payload->hit_obj = object.get();  // 获取对象指针,unique_ptr.get()返回指针
...
Vector3f castRay(const Vector3f &orig, const Vector3f &dir, const Scene &scene,
    ...
    // 语法糖,就是先初始化payload,再if(payload)判断是否有值
    if (auto payload = trace(orig, dir, scene.get_objects()); payload) {
        ...
        switch (payload->hit_obj->materialType) {
            ...
            case REFLECTION_AND_REFRACTION: {
                ...
                Vector3f refractionRayOrig =
                    (dotProduct(refractionDirection, N) < 0)
                        ? hitPoint - N * scene.epsilon
                        : hitPoint + N * scene.epsilon;
                Vector3f reflectionColor = castRay(
                    reflectionRayOrig, reflectionDirection, scene, depth + 1);
            ...
            default: {
                ...
                // up We also apply the lambert cosine law
                // [/comment]
                for (auto &light : scene.get_lights()) {
                    ...
                    bool inShadow =
                        shadow_res && (shadow_res->tNear * shadow_res->tNear <
                                       lightDistance2);

                    lightAmt += inShadow ? 0 : light->intensity * LdotN;  // 只要垂直方向
                    Vector3f reflectionDirection = reflect(-lightDir, N);

                    specularColor +=
                        powf(std::max(0.f,
                                    -dotProduct(reflectionDirection, dir)),
                                    payload->hit_obj->specularExponent) * light->intensity;
                }

                hitColor = lightAmt * payload->hit_obj->evalDiffuseColor(st) *
                               payload->hit_obj->Kd + specularColor * payload->hit_obj->Ks;
                break;
            }
        }
    }
...
void Renderer::Render(const Scene &scene) {
    ...
    for (int j = 0; j < scene.height; ++j) {
        for (int i = 0; i < scene.width; ++i) {
            ...
            float nx = (i + 0.5f) * 2 / scene.width - 1.0f;  // 使得光线对准像素中心
