...
class Bounds3 {
   public:
    ...
    // 将 3D 空间中的点 p 转换为归一化坐标，即 相对于 AABB（轴对齐包围盒，Bounds3）的位置比例。
...
inline bool Bounds3::IntersectP(const Ray& ray, const Vector3f& invDir,
    ...
    for (int i = 0; i < 3; i++) {
        float min = (pMin[i] - origin[i]) * invDir[i];  // x = o + t * d
        float max = (pMax[i] - origin[i]) * invDir[i];
        // 如果光线方向是负的，交换min和max
        if (!dirIsNeg[i]) {
            std::swap(min, max);
        }
        tEnter = std::max(min, tEnter);    // 进入时间是三个最小值中最大的
        tExit = std::min(max, tExit);      // 离开时间是三个最大值中最小的
    }
